            ...
            try
            {
                ...
                for (int i = 0; i < v_arrNK_ID.Length; i++)
                {
                    try
                    {
                        ...
                        //if (v_objData.Trang_Thai_Xuat_Kho_ID != (int)ETrang_Thai_Xuat_Kho.New)
                        //    throw new Exception($"Phiếu nhập [{v_objData.So_Phieu_Xuat}] khác trạng thái kế hoạch");

                        //v_objData.Trang_Thai_Xuat_Kho_ID = (int)ETrang_Thai_Xuat_Kho.Da_Nhan;
                        //v_objData.Last_Updated_By = User_Name;
                        //v_objData.Last_Updated_By_Function = Function_Code;
                ...
                FCommonFunction.Show_Message_Box("Thông báo", $"Soạn hàng với {v_iCount_Success} phiếu thành công.", (int)EMessage_Type.Success);
                Load_Data();
            }
            catch (Exception ex)
            {
